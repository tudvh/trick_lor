    ...
    <div class="form-container d-flex flex-column gap-3">
        ...
        <form class="form d-flex flex-column gap-4" wire:submit="register">
            <div class="form-group">
                ...
                <input type="text" id="full-name" class="@error('fullName') is-invalid @enderror"
                    placeholder="Nhập họ và tên..." wire:model="fullName">
                @error('fullName')
            ...
            <div class="form-group">
                ...
                <input type="password" id="password-confirm" class="@error('passwordConfirm') is-invalid @enderror"
                    placeholder="Nhập mật khẩu xác nhận..." autocomplete="off" wire:model="passwordConfirm">
        ...
        <p class="signup">Đã có tài khoản?
            <button type="button" wire:click="$parent.switchScreen('login')"> ĐĂNG NHẬP</button>
        </p>
    </div>

    <div class="loading-overlay" wire:loading wire:target="register" wire:loading.class="d-flex">
...
@script
    <script>
        $wire.on('register-success', async () => {
            await Swal.fire({
                icon: 'success',
                title: 'Thành công',
                text: 'Chúng tôi đã gửi một email cho bạn. Vui lòng kiểm tra và kích hoạt tài khoản của bạn.',
            })
