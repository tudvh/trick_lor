...
@section('css')
    ...
    <link rel="stylesheet" href="{{ url('site/css/post.css') }}">
    <link rel="stylesheet" href="{{ url('site/css/post-detail.css') }}">
    <link rel="stylesheet" href="{{ url('site/css/post-comment.css') }}">
@stop
        ...
        <div class="d-flex flex-column gap-5 card">
            <div class="post-header">
                ...
                <div class="post-category d-flex flex-wrap gap-2 mt-2">
                    @foreach ($post->categories as $category)
                        <a href="{{ route('site.category', ['category' => $category->slug]) }}" class="icon-box"
                ...
                <div class="d-flex align-items-center gap-3 mt-3">
                    <a href="{{ route('site.profile', ['username' => $post->author->username]) }}"
                    ...
                    <div class="d-flex flex-column justify-content-center">
                        <a href="{{ route('site.profile', ['username' => $post->author->username]) }}" class="fw-bold m-0"
                            title="{{ $post->author->full_name }}">
                            {{ $post->author->full_name }}
                        </a>
                        <span class="post-info">
                            {{ NumberHelper::format($post->post_views_count) }} lượt xem •
                            {{ DateHelper::formatTimeAgo($post->created_at) }}
                        </span>
                    </div>
                    <div class="post-more ms-auto d-flex">
                        <livewire:site.post.post-save :postId="$post->id" />
                        <button type="button" class="dropdown-toggle post-more-btn" data-bs-toggle="dropdown"
                            aria-expanded="false" title="Chia sẻ">
                            <i class="fa-sharp fa-solid fa-share-nodes"></i>
                        </button>
                        <ul class="dropdown-menu dropdown-menu-end">
                            <li>
                                <button id="copy-current-link"
            ...
            @if ($post->youtube_id)
                <div class="video-container">
                    <iframe class="post-video" type="text/html" frameborder="0"
                        src="https://www.youtube-nocookie.com/embed/{{ $post->youtube_id }}?autoplay=1&iv_load_policy=3&loop=1&playlist={{ $post->youtube_id }}&modestbranding=1&rel=0&showinfo=0&vq=hd1080"
...
@section('js')
    ...
    <!-- Local JS -->
    <script src="{{ url('assets/js/copy-code.js') }}"></script>
    <script src="{{ url('site/js/post.js') }}"></script>
@stop
